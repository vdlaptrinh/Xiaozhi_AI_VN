...
- 项目自有 C++ 代码以 GNU C++23 为基线，`main/` 中不得依赖更高版本的语言特性。
- 关闭 C++ 异常和 RTTI，不使用 `throw`、`try`/`catch`、`dynamic_cast` 或 `typeid`。
- 所有权资源使用 RAII，包括需要显式释放的 ESP-IDF 句柄和 C API 资源。除非接口明确说明，
  裸指针均视为非拥有指针。
- 需要携带错误值的可恢复失败使用 `std::expected<T, E>`。高频路径优先使用小型枚举或
  `esp_err_t`，只在协议或 UI 边界转换为字符串。
- 解引用 `expected` 前必须检查状态。异常关闭时，不得对未检查的结果调用 `value()`，
  否则错误访问会直接终止固件。
- 程序员不变量被破坏时使用断言或显式终止，不把它伪装成可恢复运行时错误。
- 实时音频路径使用有界队列并避免重复分配。上限已知时优先固定容量存储；只有性能分析
  证明存在分配抖动或堆碎片后才引入对象池。
