...
typedef struct {
    const st7121_lcd_init_cmd_t*
        init_cmds; /*!< Pointer to initialization commands array. Set to NULL if using default
                    * commands. The array should be declared as `static const` and positioned
                    * outside the function. Please refer to `vendor_specific_init_default` in source
                    * file.
                    */
 ...
 *
 * @note  Vendor specific initialization can be different between manufacturers, should consult the
 ...
 *
 * @note  refresh_rate = (dpi_clock_freq_mhz * 1000000) / (h_res + hsync_pulse_width +
 * hsync_back_porch + hsync_front_porch) / (v_res + vsync_pulse_width + vsync_back_porch +
 * vsync_front_porch)
 *
