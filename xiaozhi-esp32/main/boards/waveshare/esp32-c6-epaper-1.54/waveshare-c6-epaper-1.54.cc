...
private:
    ...
    void InitializeIoExpander() {
        ESP_ERROR_CHECK(
            esp_io_expander_new_i2c_tca9554(i2c_bus_, IO_EXPANDER_I2C_ADDRESS, &io_expander_));

        ESP_ERROR_CHECK(
             esp_io_expander_set_dir(io_expander_, IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1 | IO_EXPANDER_PIN_NUM_3 | IO_EXPANDER_PIN_NUM_4 | IO_EXPANDER_PIN_NUM_5, IO_EXPANDER_OUTPUT));
    }
    ...
    void InitializeLcdDisplay() {
        ...
        display_ = new CustomLcdDisplay(nullptr, nullptr, EXAMPLE_LCD_WIDTH, EXAMPLE_LCD_HEIGHT,
                                        DISPLAY_OFFSET_X, DISPLAY_OFFSET_Y, DISPLAY_MIRROR_X,
                                        DISPLAY_MIRROR_Y, DISPLAY_SWAP_XY, lcd_spi_data);
    }

    void Power_Init() {
        power_ = new BoardPowerBsp(io_expander_, EPD_PWR_PIN, AUDIO_PWR_PIN, VBAT_PWR_PIN,
                                   PA_CTRL_PIN, LED_G_PIN);
        power_->VbatPowerOn();
        power_->PowerAudioOn();
        power_->PowerEpdOn();
        do {
    ...
    uint16_t BatterygetVoltage(void) {
        ...
        if (!initialized) {
            ...
            ch_config.atten = ADC_ATTEN_DB_12;
            ch_config.bitwidth = ADC_BITWIDTH_12;
